...
void main()
{
	...
	// vout.texcoord = vec2(texcoord.x, 1-texcoord.y);
	...
	vout.position = vec3(uTransform * vec4(position, 1.0));
	vout.texcoord = vec2(texcoord.x, 1.0-texcoord.y);

	// Pass tangent space basis vectors (for normal mapping).
	vout.tangentBasis = mat3(uTransform) * mat3(tangent, bitangent, normal);

	gl_Position = viewProjectionMatrix * uTransform * vec4(position, 1.0);
}
...
void main()
{
	...
	vec3 albedo = texture(albedoTexture, vin.texcoord).rgb * uMaterial.AlbedoColor;
	float metalness = texture(metalnessTexture, vin.texcoord).r * uMaterial.Metalness;
	float roughness = texture(roughnessTexture, vin.texcoord).r * uMaterial.Roughness;
