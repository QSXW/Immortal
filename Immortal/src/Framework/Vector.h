...
namespace Immortal {
	...
	namespace Vector
	{
		...
		template <class T>
		constexpr __forceinline auto Normalize(T &v)
		{
			return glm::normalize(v);
		}

		template <class T>
		constexpr __forceinline auto Cross(T &v1, T &v2)
		{
			return glm::cross(v1, v2);
		}

		__forceinline auto Radians(float degrees)
		{
			return glm::radians(degrees);
		}

		__forceinline auto Radians(Vector3 degrees)
		{
			return glm::radians(degrees);
		}

		__forceinline auto Degrees(float radians)
		{
			return glm::degrees(radians);
		}

		__forceinline auto Degrees(Vector3 radians)
		{
			return glm::degrees(radians);
		}

		__forceinline auto Translate(Vector3 position)
		{
			return glm::translate(mat4(1.0f), position);
		}

		__forceinline auto Rotate(float rotation, Vector3 direction)
		{
		...
		template <class T>
		__forceinline auto Rotate(Quaternion &q, T v)
		{
			return glm::rotate(q, v);
		}

		__forceinline auto ToMatrix4(Quaternion &q)
		{
			return glm::toMat4(q);
		}

		__forceinline auto Rotate(Vector3 rotation)
		{
			return glm::toMat4(Quaternion(rotation));
		}

		__forceinline auto Scale(Vector3 scala)
		{
			return glm::scale(mat4(1.0f), scala);
		}

		__forceinline auto Inverse(mat4 matrix)
		{
			return glm::inverse(matrix);
		}

		__forceinline auto Ortho(float left, float right, float bottom, float top, float zNear, float zFar)
		{
			return glm::ortho(left, right, bottom, top, zNear, zFar);
		}

		__forceinline auto PerspectiveFOV(float fov, float width, float height, float zNear, float zFar)
		{
			return glm::perspectiveFov(fov, width, height, zNear, zFar);
		}

		__forceinline auto EpsilonEqual(const float &x, const float &y, const float &epsilon)
		{
			return glm::epsilonEqual(x, y, epsilon);
		}

		__forceinline auto EpsilonNotEqual(const float &x, const float &y, const float &epsilon)
		{
			return glm::epsilonNotEqual(x, y, epsilon);
		}

		template <class T>
		__forceinline auto Distance(T &p0, T &p1)
		{
			return glm::distance(p0, p1);
		}

		template <class T>
		__forceinline auto EulerAngles(T &x)
		{
			return glm::eulerAngles(x);
		}

		template <class T>
		__forceinline auto Epsilon()
		{
			return glm::epsilon<T>();
		}

		template <class T>
		__forceinline auto Length(T &q)
		{
			return glm::length(q);
		}
