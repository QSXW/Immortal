...
namespace Immortal {
	...
	static GLenum ShaderTypeFromString(const std::string &type)
	{
		...
		IM_CORE_ASSERT(false, "Unknown shader type!");
		return 0;
	}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& source)
	{
		...
		for (auto &src : source)
		{
			...
			glAttachShader(mRendererID, shaderIDs[index]);
			index++;
		}
		...
		{
			GLint maxLength = 0;
			glGetProgramiv(mRendererID, GL_INFO_LOG_LENGTH, &isLinked);

			Scope<GLchar> infoLog = CreateScope<GLchar>(maxLength);
			glGetProgramInfoLog(mRendererID, maxLength, &maxLength, infoLog.get());
			...
			IM_CORE_ASSERT(isLinked == GL_FALSE, "Shader compilation failure!");
		}

		for (int i = 0; i < source.size(); i++)
		{
			glDetachShader(mRendererID, shaderIDs[i]);
		}	
	}
	...
	uint32_t OpenGLShader::CompileShader(int type, const char *src)
	{
		...
		if (isCompiled == GL_FALSE)
		{
			...
			IM_CORE_ERROR("{0}", infoLog.get());
			IM_CORE_ASSERT(isCompiled, "Shader compilation failure!");
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::Preprocess(const std::string & source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			IM_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
			std::string type = source.substr(begin, eol - begin);
			IM_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
			IM_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	{
		IM_CORE_ASSERT(mType == Shader::Type::Compute, "The shader type is not compute.");
