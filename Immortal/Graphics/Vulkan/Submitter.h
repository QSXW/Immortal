...
struct TimelineSubmitter
{
    ...
    operator const VkTimelineSemaphoreSubmitInfo&()
    {
        ...
        submitInfo.waitSemaphoreValueCount = uint32_t(waitValues.size());
        submitInfo.pWaitSemaphoreValues    = waitValues.data();

        submitInfo.signalSemaphoreValueCount = uint32_t(signalValues.size());
...
public:
    ...
    {
		handle.waitSemaphoreCount = uint32_t(waitSemaphores.size());
        handle.pWaitSemaphores    = waitSemaphores.data();

        handle.swapchainCount = uint32_t(swapchains.size());
