...
static void DrawTreeNode(BoneNode *node)
{
	...
	{
		ImGui::BulletText("%s", node->Name.empty() ? "<node>" : node->Name.c_str());
		return;
	}
	if (ImGui::TreeNodeEx((void *)(uintptr_t)node, ImGuiTreeNodeFlags_None, "%s", node->Name.empty() ? "<node>" : node->Name.c_str()))
	{
                ...
                if (object.HasComponent<MaterialComponent>() && object.HasComponent<MeshComponent>())
                {
                    DrawComponent(
                        ...
                        [&]() -> void {
                            ...
                            if (nodeList.empty() || material.References.empty())
                            {
                                return;
                            }

                            const size_t nodeCount = std::min(nodeList.size(), material.References.size());
                            if (nodeCount == 0)
                            {
                                return;
                            }

                            uint32_t refIndex = mesh.SelectedDrawNodeIndex;
                            if (refIndex == UINT32_MAX || refIndex >= nodeCount)
                            {
                                refIndex = 0;
                            }
                            const int materialIndex = (int)refIndex;

                            static ImGuiComboFlags flags = 0;
                            const char *previewValue = nodeList[(size_t)materialIndex].Name.c_str();
                            ...
                            UILock lock{
                                ...
                                [&] {
                                    ...
                                    if (ImGui::BeginCombo("###", previewValue, flags))
                                    {
                                        ...
                                        {
                                            const bool isSelected = (materialIndex == (int)i);
                                            if (ImGui::Selectable(nodeList[i].Name.c_str(), isSelected))
                                            {
                                                mesh.SelectedDrawNodeIndex = (uint32_t)i;
                                            }
                                            if (isSelected)
                                            {
                            ...
                            auto &ref = material.References[(size_t)materialIndex];
                            ...
                            auto drawTexture = [&](Ref<Texture> &texture, const std::string &label) -> void {
								...
								if (ImGui::ImageButton("Texture", WIMAGE(texture), size))
                                {
                            ...
                            UILock rootNodeLock{
                                ...
                                [&] {
                                    ...
                                    if (ImGui::TreeNodeEx("##bone_hierarchy", ImGuiTreeNodeFlags_DefaultOpen, "%s",
                                                          Translator::Translate("Skeleton").c_str()))
                                    {
