	...
	virtual bool Draw() override
    {
...

		{
			...
			for (int i = 0; i < SL_ARRAY_LENGTH(vOffset.tool); i++)
			{
				...
				ImGui::PushID(i);
				if (ImGui::ImageButton("###", WIMAGE(texture), size, uv0, uv1, backgroundColor, tintColor))
				{
					Activate(i);
				}
			...
			ImGui::PushFont(GuiLayer::NotoSans.Bold);
			{
				...
				if (ImGui::Button(Translator::Translate("Button1").c_str(), ImVec2{size.x * 2.4f, size.y}))
				{
				}
				ImGui::SameLine(0.0f, 1.0);
				if (ImGui::Button(Translator::Translate("Button2").c_str(), ImVec2{size.x * 2.4f, size.y}))
				{
				}
			}
			ImGui::PopFont();
