// Editor grid underlay: blend onto main RT before forward DrawMesh. Entry: VSMain / PSMain.
...
Texture2D    BindGuardTex : register(t0);
SamplerState BindGuardSamp : register(s1);
...
struct PSOutUnderlay
{
	float4 color : SV_Target0;
	uint2  objectId : SV_Target1;
};
...
// Screen-space analytic AA: fwidth + smoothstep on grid coordinates (reduces moiré / shimmer).
...
PSOutUnderlay PSMain(PSInput input)
{
	PSOutUnderlay outv;
	outv.objectId = uint2(0u, 0u);
	...
	// Keep Sample live so DXIL retains t0/s1 for D3D12 reflection (avoid DCE of *0).
	float3 bindTouch = BindGuardTex.Sample(BindGuardSamp, input.uv).rgb;
	outv.color = float4(lineRgb + bindTouch * 1e-8f, a);
	return outv;
}
